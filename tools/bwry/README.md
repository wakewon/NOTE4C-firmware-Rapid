...
| 5 自适应 palette look（可选） | 先估计保真映射会损失多少色彩，再把蓝/绿等不可表示色统一翻译到砖红—赭黄 | 已接近面板色域的图只受轻微调色；不可表示色占得越多，老照片风格越完整 |
...
1. 先用实测面板色域做一次无风格的硬映射，以 `ΔE × 彩色区域占比` 计算整图
   `unprintable_severity`；
2. 再用这个全局强度和逐像素损失图驱动可烘焙的 `vintage` look。可打印红/黄区域有局部保护，
   大面积蓝天、绿地才会完整转成统一的砖红—赭黄语言。
