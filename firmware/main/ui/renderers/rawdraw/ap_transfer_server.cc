...
function rgba(c){return c===0?[70,64,69]:c===1?[255,255,255]:c===2?[255,233,30]:[199,75,75]}
...
function fitImage(file,cover){return new Promise((res,rej)=>{const img=new Image();img.onload=()=>{const c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d',{willReadFrequently:true});x.fillStyle='#fff';x.fillRect(0,0,W,H);const s=(cover?Math.max:Math.min)(W/img.width,H/img.height),w=Math.round(img.width*s),h=Math.round(img.height*s);x.drawImage(img,(W-w)/2,(H-h)/2,w,h);URL.revokeObjectURL(img.src);res(x.getImageData(0,0,W,H).data)};img.onerror=rej;img.src=URL.createObjectURL(file)})}
function convert09k(data){if(typeof Worker==='undefined')return Promise.resolve(Bwry09k.rgbaToBwry2bpp(data,W,H,{onProgress:(stage)=>statusEl.textContent=`09k 转换中 · ${stage}`}));return new Promise((resolve,reject)=>{const core=document.getElementById('bwryCore').textContent,workerCode=core+`\nonmessage=e=>{try{const out=Bwry09k.rgbaToBwry2bpp(new Uint8Array(e.data),400,300,{onProgress:stage=>postMessage({stage})});postMessage({out:out.buffer},[out.buffer])}catch(error){postMessage({error:String(error&&error.message||error)})}}`;const url=URL.createObjectURL(new Blob([workerCode],{type:'text/javascript'})),worker=new Worker(url);worker.onmessage=e=>{if(e.data.stage){statusEl.textContent=`09k 转换中 · ${e.data.stage}`;return}worker.terminate();URL.revokeObjectURL(url);if(e.data.error)reject(new Error(e.data.error));else resolve(new Uint8Array(e.data.out))};worker.onerror=e=>{worker.terminate();URL.revokeObjectURL(url);reject(e.error||new Error(e.message))};worker.postMessage(data.buffer,[data.buffer])})}
async function convert(file){const mode=fmt(),data=await fitImage(file,mode==='bwry2bpp');pendingFmt=mode;if(mode==='bwry2bpp'){const out=await convert09k(data);draw2(out,pv);return out}const gray=new Int16Array(W*H);for(let p=0,i=0;p<gray.length;p++,i+=4)gray[p]=(data[i]*30+data[i+1]*59+data[i+2]*11)/100|0;const out=new Uint8Array(15000);for(let y=0;y<H;y++)for(let x=0;x<W;x++){const p=y*W+x,old=Math.max(0,Math.min(255,gray[p])),nw=old>128?255:0,err=old-nw;if(nw>128)out[p>>3]|=1<<(7-(p&7));if(x+1<W)gray[p+1]+=err*7/16;if(y+1<H){if(x>0)gray[p+W-1]+=err*3/16;gray[p+W]+=err*5/16;if(x+1<W)gray[p+W+1]+=err/16}}draw1(out,pv);return out}
